...
Headers::iterator Headers::find(const std::string& key) {
  ...
  for (i = headers_.begin(); i < headers_.end(); i++)
    if (i->keyMatches(key)) break;
  return i;
}

Headers::iterator Headers::begin() { return headers_.begin(); }

Headers::iterator Headers::end() { return headers_.end(); }

Headers::citerator Headers::cbegin() const { return headers_.begin(); }

Headers::citerator Headers::cend() const { return headers_.end(); }

Headers::iterator Headers::find_or_create(const std::string& key) {
  iterator found = find(key);
  if (found != headers_.end()) return found;
  return headers_.insert(headers_.end(), Header(key));
}

Headers::Header::Header(const std::string& key) : key_(key), values_() {
  for (size_t i = 0; i < key_.length(); ++i) key_[i] = static_cast<char>(std::tolower(key[i]));
}

bool Headers::Header::keyMatches(const std::string& key) const {
  if (key.length() != key_.length()) return false;
  for (size_t i = 0; i < key.length(); ++i)
    if (key_[i] != std::tolower(key[i])) return false;
  return true;
}
