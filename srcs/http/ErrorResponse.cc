...
const std::string& ErrorResponse::errpage_template = "<!DOCTYPE html>"\
...
ErrorResponse::ErrorResponse(int error) throw() {
  ...
  std::snprintf(body, len + 1, errpage_template.c_str(),
                error, reason.c_str(), reason.c_str());
  setBody(body, len);
}

ErrorResponse::~ErrorResponse() throw() {}

ErrorResponse::ErrorResponse(const ErrorResponse& other) throw()
    : Response(other) {}
