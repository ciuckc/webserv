...
class BufferPool {
 ...
 public:
  struct BorrowedBuffer {
   ...
   public:
    explicit BorrowedBuffer(std::list<char*>& owner) : owner_(owner), data_(owner.front()) {
      owner.pop_front();
    };
    BorrowedBuffer(BorrowedBuffer&& other) noexcept : owner_(other.owner_), data_(other.data_) {
      other.data_ = nullptr;
    };
    ~BorrowedBuffer() {
      if (data_ != nullptr)
