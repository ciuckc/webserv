...
class ConnectionBuffer {
 private:
  ...
  static constexpr size_t max_file_bufs_ = 0xFFFF / buf_size_;

  struct Buf {
    using type = std::array<char, buf_size_>;

    type data_;

    inline Buf::type& getData() {
      return data_;
    }
    inline std::string_view getView(size_t offset, size_t len = std::string::npos) {
      return {data_.data() + offset, std::min(len, buf_size_ - offset)};
    }
  };
 ...
 public:
  ...
  inline size_t inAvailable() const { return i_end_ - i_offset_ + 1; }
  ...
  inline size_t outAvailable() const { return buf_size_ - o_offset_ + 1; }
